...
export class AddMatchComponent implements AfterContentChecked, OnInit {
  ...
  addSingleMatchForm = this.fb.group({
    players: this.fb.group({
      team1: this.fb.group(
        {
          one: [null, { validators: Validators.required }],
  ...
  ngOnInit(): void {
    this.currentForm = this.singleMode
      ? this.addSingleMatchForm
      : this.addMatchForm;
  ...
  public async saveMatch() {
    if (
      (this.singleMode && !this.singleModeSaveButtonDisabled) ||
      (!this.singleMode && !this.playersSaveButtonDisabled)
    ) {
      if (!this.singleMode) {
        ...
        this.matchesService.add(this.currentForm).then(() => {
          this.resetForm();
        });
      } else {
        this.singleModeSaveButtonDisabled = true;
        this.matchesService.addSingleMatch(this.currentForm).then(() => {
          this.resetForm();
        });
      }
    }
  ...
  resetForm() {
    ...
    this.singleMode
      ? (this.singleModeSaveButtonDisabled = false)
