...
async function updateUsersAndTeamsProcedure(snap: functions.firestore.QueryDocumentSnapshot) {
    ...
    if (team1Counter > 0) {
        team1UpdateMap.set('dominations', admin.firestore.FieldValue.increment(team1Counter));
    }

    if (team2Counter > 0) {
        team2UpdateMap.set('dominations', admin.firestore.FieldValue.increment(team2Counter));
    }
    ...
    if (team1Counter > 0) {
        team1UpdateMap.set('defeats', admin.firestore.FieldValue.increment(team1Counter));
    }

    if (team2Counter > 0) {
        team2UpdateMap.set('defeats', admin.firestore.FieldValue.increment(team2Counter));
    }
...
exports.updateUsersAndTeamsT = functions.firestore.document(`T-Matches/{documentId}`).onCreate((snap) => {
...
exports.updateUsersAndTeams = functions.firestore.document(`Matches/{documentId}`).onCreate((snap) => {
    updateUsersAndTeamsProcedure(snap);
    return null;
});